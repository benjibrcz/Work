...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Time iteration equations for all of the variable fields */   
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void df_dt(double cos_const, __complex__ double ****alpha_AB_pi_mode, Classical_fields *c_fields, Classical_fields *c_fields_dot, Quantum_fields **q_fields, Quantum_fields **q_fields_dot, Metric_Fields *metric, Metric_Fields *metric_dot){
    ...
    #pragma omp parallel for //num_threads(lattice_size_buff)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Function that calculates the trace of the stress-energy tensor */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
double calc_trace(int i, double cos_const, Classical_fields* c_fields, Quantum_fields** q_fields, Metric_Fields* metric) {
    ...
    rho = 1.0 / (2.0 * A) * (pi_pi / (B * B) + chi_chi) + 1.0 / B * del_theta_phi_del_theta_phi_over_r_sq - cos_const;
    S_A = 1.0 / (2.0 * A) * (pi_pi / (B * B) + chi_chi) - 1.0 / B * del_theta_phi_del_theta_phi_over_r_sq + cos_const;
    S_B = 1.0 / (2.0 * A) * (pi_pi / (B * B) - chi_chi) + cos_const;

    return (-rho + S_A + 2.0 * S_B);
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Function that calculates the Ricci scalar */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
double calc_ricci_scalar(int i, Classical_fields *c_fields, Metric_Fields *metric) {
    ...
    ricci_scalar =  D_B_deriv / B - 3.0 * (U_tilda_deriv + 2.0 * D_B_deriv + 4.0 * B * lambda_deriv / A) / A + 8.0 * D_B_deriv / A + 2.0 * K_B * (3.0 * K_B - 2 * K);
...
#pragma omp parallel for
            ...
            for (int k = 0; k < number_of_k_modes; ++k) {
                for (int l = 0; l < number_of_l_modes; ++l) {
                    for (int which_q_field = 0; which_q_field < number_of_q_fields; ++which_q_field) {
                        ...
                        Phi_mode1 = r_l1 * (q_fields[which_q_field]->pi[k][l][i]);
    ...
    if (n == 0) {
        FILE* finout;
        finout = fopen("correlator_pipi_q100_noghost_1.txt", "w");
        ...
        FILE* finout;
        finout = fopen("correlator_pipi_q100_noghost_2.txt", "w");
        for (int i = 0; i < lattice_size; ++i) {
            fprintf(finout, "\n");
            for (int j = 0; j < lattice_size; ++j) {
                fprintf(finout, "%.20f ", (correlator_half[i][j]));
        ...
        FILE* finout;
        finout = fopen("correlator_pipi_q100_noghost_3.txt", "w");
        for (int i = 0; i < lattice_size; ++i) {
            fprintf(finout, "\n");
            for (int j = 0; j < lattice_size; ++j) {
                fprintf(finout, "%.20f ", (correlator_half[i][j]));
        ...
        FILE* finout;
        finout = fopen("correlator_pipi_q100_noghost_4.txt", "w");
        for (int i = 0; i < lattice_size; ++i) {
            fprintf(finout, "\n");
            for (int j = 0; j < lattice_size; ++j) {
                fprintf(finout, "%.20f ", (correlator_half[i][j]));
        ...
        FILE* finout;
        finout = fopen("correlator_pipi_q100_noghost_5.txt", "w");
        for (int i = 0; i < lattice_size; ++i) {
            fprintf(finout, "\n");
            for (int j = 0; j < lattice_size; ++j) {
                fprintf(finout, "%.20f ", (correlator_half[i][j]));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Function that saves correlation function at the end of simulation from r=-r_max to r=r_max */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void save_correlator(Classical_fields* c_fields, Quantum_fields** q_fields, Metric_Fields* metric) {

    

    double correlator[2*lattice_size][2*lattice_size];
    #pragma omp parallel
    for (int i = -lattice_size + 1; i < lattice_size; ++i) {
        for (int j = -lattice_size + 1; j < lattice_size; ++j) {
            ...
            //phi1=c_fields->phi[i-buff_size];
           // phi2=c_fields->phi[j-buff_size];
            ...
            for (int k = 0; k < number_of_k_modes; ++k) {
                for (int l = 0; l < number_of_l_modes; ++l) {
                    for (int which_q_field = 0; which_q_field < number_of_q_fields; ++which_q_field) {
                        ...
                        r_l2 = pow(r2, l_value);
                        if (i < -buff_size) {
                            Phi_mode1 = r_l1 * (q_fields[which_q_field]->phi[k][l][-i + buff_size]);
                        }
                        else {
                            Phi_mode1 = r_l1 * (q_fields[which_q_field]->phi[k][l][i + buff_size]);
                        }
                        if (j < -buff_size) {
                            Phi_mode2 = r_l2 * (q_fields[which_q_field]->phi[k][l][-j + buff_size]);
                        }
                        else {
                            Phi_mode2 = r_l2 * (q_fields[which_q_field]->phi[k][l][j + buff_size]);
                        }
                        ...
                        phi_phi   = phi_phi   + hbar * ghost_or_physical[which_q_field] * dk / (4.0 * PI) * (2.0 * l_value + 1.0) * __real__(Phi_mode1 * conj(Phi_mode2));
                        //phi1_phi1 = phi1_phi1 + hbar * ghost_or_physical[which_q_field] * dk / (4.0 * PI) * (2.0 * l_value + 1.0) * __real__(Phi_mode1 * conj(Phi_mode1));
                        //phi2_phi2 = phi2_phi2 + hbar * ghost_or_physical[which_q_field] * dk / (4.0 * PI) * (2.0 * l_value + 1.0) * __real__(Phi_mode2 * conj(Phi_mode2));
                    }
                }
            }
            correlator[i + lattice_size - 1][j + lattice_size - 1] = phi_phi;
        }
    }

    FILE* finout;
    finout = fopen("correlator_new.txt", "w");
    for (int i = 0; i < 2 * lattice_size - 1; ++i) {
        fprintf(finout, "\n");
        for (int j = 0; j < 2 * lattice_size - 1; ++j) {
            fprintf(finout, "%.20f ", (correlator[i][j]));
        }
    }
    fclose(finout);
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Function that calculates the (t,t) component of the Einstein tensor */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
double G_tt(double cos_const, int i, Classical_fields *c_fields, Quantum_fields** q_fields, Metric_Fields *metric) {
    double Gtt;
    ...
    K_B_dot = (i != buff_size ? alpha / (r * A) * (0.5 * U_tilda + 2.0 * lambda * B / A - D_B - lambda - D_alpha) : alpha / (A) * (0.5 * first_deriv(i, metric->U_tilda)
        + (2.0 * B / A - 1.0) * first_deriv(i, metric->lambda)
        - first_deriv(i, metric->D_B)
        - first_deriv(i, metric->D_alpha)))
        + alpha / A * (-0.5 * D_alpha * D_B
    ...
    Gtt = (2.0 + D_B * r) * (2.0 + 4.0 * r * D_alpha + r * D_B) / (4 * r * r) - A * (1 / (r * r * B) + 3.0 * K_B * K_B - 2.0 * K_B_dot / alpha);
    return Gtt;
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Full evolution function */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int fullEvolution(double b_i[nu_legendre], double a_ij[nu_legendre][nu_legendre], __complex__ double ****alpha_AB_pi_mode, Classical_fields *c_fields_RK1, Classical_fields *c_fields_RK2, Classical_fields *c_fields_RK3,
                                    Classical_fields *c_fields_RK4, Classical_fields *c_fields_RK5, Classical_fields *c_fields_RK_sum, Classical_fields *c_fields,
                                    Quantum_fields **q_fields_RK1, Quantum_fields **q_fields_RK2, Quantum_fields **q_fields_RK3, Quantum_fields **q_fields_RK4,
                                    Quantum_fields **q_fields_RK5, Quantum_fields **q_fields_RK_sum, Quantum_fields **q_fields,
                                    Metric_Fields *metric_RK1, Metric_Fields *metric_RK2, Metric_Fields *metric_RK3, Metric_Fields *metric_RK4,
    ...
    double mass_ADM[lattice_size_buff];
    ...
    //give the u and pi a starting point for the iterations involved in finding the implicit evolution step
           ...
           // }
            ...
            //}
            /*if (n == 300) {
                save_correlator_half(0,q_fields, correlator_half);
            }
            if (n == 500) {
                save_correlator_half(1,q_fields, correlator_half);
            }
            if (n == 750) {
                save_correlator_half(2,q_fields, correlator_half);
            }
            if (n == 950) {
...
void main(){
    ...
    //allocate memory for classical fields, accessed with c_fields->phi[i]
    ...
    set_A_U_lambda_initial          (cosm_const, c_fields,   q_fields,   metric);
    ...
    fullEvolution(b_i, a_ij, alpha_AB_pi_mode, c_fields_RK1, c_fields_RK2, c_fields_RK3, c_fields_RK4, c_fields_RK5, c_fields_RK_sum, c_fields,
                 q_fields_RK1, q_fields_RK2, q_fields_RK3, q_fields_RK4, q_fields_RK5, q_fields_RK_sum, q_fields,
                 metric_RK1, metric_RK2, metric_RK3, metric_RK4, metric_RK5, metric_RK_sum, metric, alpha_save, cosm_const, timestart, q_fields_t_star, correlator_half, field_save);
    
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////* SAVE ALL RELEVANT DATA *///////////////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    
    save_correlator_half(4, q_fields, correlator_half);
    
    double rho[lattice_size_buff];
    double r[lattice_size_buff];
    make_points(r);
    //#pragma omp parallel for
    double massadm[lattice_size_buff];
